...
static void mouseCallback(GLFWwindow *window, double xpos, double ypos) {
  if (!appModule.cursorSnap) {
    appModule.prevX = xpos;
    appModule.prevY = ypos;
    appModule.cursorSnap = true;
  }
  const float dx = (float)(appModule.prevX - xpos);
  const float dy = (float)(appModule.prevY - ypos);
  appModule.prevX = xpos;
  appModule.prevY = ypos;

  const float magnitude =
      (float)(appModule.updateCycle.delta * appModule.scene.camera.sensitivity);
  appModule.scene.camera.rotate(dx * magnitude, dy * magnitude);
}
...
void App::catchException(const std::runtime_error &e) {
  std::cout << e.what();
  glfwSetWindowShouldClose(appModule.window, GL_TRUE);
}
